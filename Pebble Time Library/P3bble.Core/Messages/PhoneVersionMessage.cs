...
namespace P3bble.Messages
{
    internal class PhoneVersionMessage : P3bbleMessage
    {
        ...
        protected override void AddContentToMessage(List<byte> payload)
        {
            ...
            byte[] message = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x02, 0x03, 0x00, 0x01, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

            byte[] msg = new byte[0];
            //msg = msg.Concat(prefix).Concat(session).Concat(remote).ToArray();
            msg = msg.Concat(prefix).Concat(message).ToArray();
